...
ORG                                ?= eddycharly
PACKAGE                            ?= github.com/$(ORG)/generic-auth-server
GOPATH_SHIM                        := ${PWD}/.gopath
PACKAGE_SHIM                       := $(GOPATH_SHIM)/src/$(PACKAGE)
CLI_BIN                            := generic-auth-server
CGO_ENABLED                        ?= 0
...
codegen-helm-crds: codegen-crds ## Generate helm CRDs
	...
	@cat $(CRDS_PATH)/* \
		...
		| $(SED) -e '/^  labels:/a \ \ \ \ {{- include "generic-auth-server.labels" . | nindent 4 }}' \
 		> ./charts/generic-auth-server/templates/crds.yaml
...
codegen-schemas-openapi: $(KIND)
	...
	@mkdir -p ./.temp/.schemas/openapi/v3/apis/generic.kyverno.io
	...
	@kubectl get --raw /openapi/v3/apis/generic.kyverno.io/v1alpha1 > ./.temp/.schemas/openapi/v3/apis/generic.kyverno.io/v1alpha1.json
...
codegen-schemas-json: codegen-schemas-openapi
	...
	@rm -rf ./.schemas/json
	@openapi2jsonschema .temp/.schemas/openapi/v3/apis/generic.kyverno.io/v1alpha1.json --kubernetes --strict --stand-alone --expanded -o ./.temp/.schemas/json
	@mkdir -p ./.schemas/json
	@cp ./.temp/.schemas/json/authorizationpolicy-generic-*.json ./.schemas/json
